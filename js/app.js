...
function goToRoute(route, { fromPopstate = false, replace = false, state = {} } = {}) {
  ...
  document.getElementById("topbar-subtitle").textContent = route === "user-profile" ? "Classmate Profile" : route === "dm-thread" ? "Private Message" : route === "settings" ? "App preferences & account" : (route === "reports" || route === "admin-verified") ? "Admin only" : "Geography & Environment";
...
function openProfileDetailsModal(isFirstTime = false) {
  ...
  openModal(`
    ...
    <div class="pd-photo-picker">
      ...
      <button type="button" class="btn-outline small pd-photo-replace-btn hidden" id="pd-photo-replace-btn">Replace Photo</button>
  ...
  const replaceBtn = document.getElementById("pd-photo-replace-btn");
